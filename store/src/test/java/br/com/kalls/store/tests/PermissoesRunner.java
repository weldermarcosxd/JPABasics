...
public class PermissoesRunner {
	...
	public void testPersistence() {
		...
		Permissao permissao = em.find(Permissao.class, "Viwer");
		PessoaFisica pessoa = em.find(PessoaFisica.class, 1L);
		
		pessoa.getPermissoes().add(permissao);
		
		Validator validator = Validation.buildDefaultValidatorFactory().getValidator();
		Set<ConstraintViolation<PessoaFisica>> violations = validator.validate(pessoa);
